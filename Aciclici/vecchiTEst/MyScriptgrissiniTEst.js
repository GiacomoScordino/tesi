...
function charts(data, planeData, containerID){
   ...
   // console.log(numLevel);
    ...
    root.eachAfter(function(d){
        ...
        for(let i=0; i<d.data.Livelli.length;i++){
            if(d.data.Livelli[i]>0 && !b ){
                b=true;
                var iniziorect=i;
                var yi= (smallest_rect_height+10*(maxHeight+1))*(i)+5*d.depth;
                console.log("componente: "+d.data.Node)
                console.log("inizia a: "+i+"quindi e' alto :"+yi);
            }else if(d.data.Livelli[i]>0 && b){ 
            }else if(d.data.Livelli[i]<=0 &&b){
                var yf=(smallest_rect_height+10*(maxHeight-d.depth))*2+10*((i-1)-iniziorect)+(smallest_rect_height+10*maxHeight)*levels+12*d.depth;
                console.log("forzaroma")
                d.rettangoli[c]=[yi, yf];
                c++;
                b=false;
            }else if(i==(numLevel-1) ){
                console.log("laziomerda")
                var yf=(smallest_rect_height+10*(maxHeight-d.depth))*2+10*((i-1)-iniziorect)+(smallest_rect_height+10*maxHeight)*levels+12*d.depth;
                d.rettangoli[c]=[yi, yf];
                c++;
                b=false;
   ...
   /* root.eachAfter(function(d){
        ...
        for(let i=0; i<=numLevel;i++){
            if(d.data.Livelli[i]>0){
                ...
                var h= c* smallest_rect_height;
                svg2.append("rect")
                .attr("fill", d => colorScale(d.data.maxCoreness))
                .attr("width", d => (d.x1 - d.x0))
                .attr("height", d => (h))
                .attr("rx", 15);
                console.log("il rettangolo e' alto:", c * smallest_rect_height, "la componente e':", d.data.Node);
                c=0;
