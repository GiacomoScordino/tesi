...
graph *set_input(char *input_path)
{
    ...
    if (file != NULL)
    {
       ...
       int verticiSinistra=0; 
        ...
        printf("alla fine di tutto ne ho contati %d\n",grafo->num_vertices);
        return grafo;
    }
    else
    {
...
{
printf("il numero di nodi sono %d\n",grafo->num_vertices);
    for (int i = 1; i <= grafo->num_vertices; i++)
    {
        node_tree *n = grafo->vertices[i].v_tree;
        int c_in=grafo->vertices[i].coreness_in;
        int c_out=grafo->vertices[i].coreness_out;
        int c=grafo->vertices[i].coreness;
        if (n->id != 0)
        {
